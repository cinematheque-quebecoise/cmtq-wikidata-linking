...
const removeCQFilmsMultiplesExternalId = async () => {
  ...
  if (films) {
    const asyncRes = await Promise.all(films.map(async (film) => removeAllExternalId(film)))
  }
...
const updateCQFilmsExternalId = async() => {
  ...
  const asyncRes = await Promise.all(films.map(async (film) => updateExternalId(film)))
